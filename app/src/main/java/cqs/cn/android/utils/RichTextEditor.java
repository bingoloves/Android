...
public class RichTextEditor extends ScrollView {
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, com.sendtion.xrichtext.R.styleable.RichTextEditor);
        rtImageHeight = ta.getInteger(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_image_height, 500);
        rtImageBottom = ta.getInteger(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_image_bottom, 10);
        rtTextSize = ta.getDimensionPixelSize(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_text_size, 16);
        //rtTextSize = ta.getInteger(R.styleable.RichTextView_rt_view_text_size, 16);
        rtTextLineSpace = ta.getDimensionPixelSize(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_text_line_space, 8);
        rtTextColor = ta.getColor(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_text_color, Color.parseColor("#757575"));
        rtTextInitHint = ta.getString(com.sendtion.xrichtext.R.styleable.RichTextEditor_rt_editor_text_init_hint);
    ...
    public void setOnRtImageDeleteListener(RichTextEditor.OnRtImageDeleteListener onRtImageDeleteListener) {
    ...
    public void setOnRtImageClickListener(RichTextEditor.OnRtImageClickListener onRtImageClickListener) {
    ...
    private void onImageCloseClick(View view) {
        try {
            if (!mTransitioner.isRunning()) {
                ...
                //删除文件夹里的图片
                List<RichTextEditor.EditData> dataList = buildEditData();
                RichTextEditor.EditData editData = dataList.get(disappearingImageIndex);
    ...
    public EditText createEditText(String hint, int paddingTop) {
        EditText editText = (EditText) inflater.inflate(com.sendtion.xrichtext.R.layout.rich_edittext, null);
    ...
    private RelativeLayout createImageLayout() {
        RelativeLayout layout = (RelativeLayout) inflater.inflate(
                com.sendtion.xrichtext.R.layout.edit_imageview, null);
        layout.setTag(viewTagIndex++);
        View closeView = layout.findViewById(com.sendtion.xrichtext.R.id.image_close);
        //closeView.setVisibility(GONE);
        closeView.setTag(layout.getTag());
        closeView.setOnClickListener(btnListener);
        DataImageView imageView = layout.findViewById(com.sendtion.xrichtext.R.id.edit_imageView);
    ...
    public void addImageViewAtIndex(final int index, final String imagePath) {
        ...
        try {
            ...
            DataImageView imageView = imageLayout.findViewById(com.sendtion.xrichtext.R.id.edit_imageView);
...
//			}, 200);
     ...
     * 对外提供的接口, 生成编辑数据上传
     */
//    public List<RichTextEditor.EditData> buildEditData() {
//        List<RichTextEditor.EditData> dataList = new ArrayList<RichTextEditor.EditData>();
//        try {
//            int num = allLayout.getChildCount();
//            for (int index = 0; index < num; index++) {
//                View itemView = allLayout.getChildAt(index);
//                RichTextEditor.EditData itemData = new RichTextEditor.EditData();
//                if (itemView instanceof EditText) {
//                    EditText item = (EditText) itemView;
//                    itemData.inputStr = item.getText().toString();
//                } else if (itemView instanceof RelativeLayout) {
//                    DataImageView item = (DataImageView) itemView.findViewById(com.sendtion.xrichtext.R.id.edit_imageView);
...
//    }
    ...
    public RelativeLayout createJzvdStd(String videoPath, String videoTitle, String thumbImageUrl) {
        ...
        if (!TextUtils.isEmpty(thumbImageUrl))
            Glide.with(Utils.getApp()).load(thumbImageUrl).into(thumbImageView);
